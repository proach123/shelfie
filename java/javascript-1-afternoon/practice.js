...
function doubleCheck(anArray){
 if (anArray.indexOf("chocolate") > -1) {
    return anArray
} else {
    anArray.push("chocolate") > -1;
    return anArray
  };
}
//Code Here
...
//Return mySum.
function looper(Arrayy){
  var mySum = 0
    for (i = 0; i < Arrayy.length; i++) { 
      if ((Arrayy[i] % 2) != 0) {
          mySum += Arrayy[i];
      } else if (Arrayy[i] >= 100){
        mySum += Arrayy[i];
      };
